...
const TableauEmbed = ({ src, title }: { src: string; title: string }) => (
  <div className="h-[600px] md:h-auto md:aspect-[16/9] w-full rounded-lg overflow-hidden border border-muted bg-muted/10 flex flex-col items-center justify-center">
    <iframe
      title={title}
      width="100%"
      height="100%"
          ...
          <Tabs defaultValue={reportes[0].name} className="w-full">
            ...
            {reportes.map((reporte) => (
              <TabsContent key={reporte.name} value={reporte.name} className="mt-4">
                <TableauEmbed src={reporte.url} title={reporte.name} />
              </TabsContent>
            ))}
          </Tabs>
        </div>
